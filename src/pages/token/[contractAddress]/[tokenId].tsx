...
import NewGalleryTab from "@components/product-details/newGallTab";
...
import ProductDetailsArea from "@containers/product-details/productDetailsArea";
...
export default function TokenPage({ nft, contractMetadata }: Props) {
  ...
  const metaDataName = contractMetadata?.name;
  ...
  const getWinningBid = async () => {
    if (auctionListing?.[0]) {
      const bid = await marketplace.englishAuctions.getWinningBid(auctionListing[0].id);
      setWinningBid(bid); // Update the winningBid state with the result
      console.log("winningBid", winningBid);
    }
  };

  // Call the getWinningBid function when the component mounts or when auctionListing changes
  useEffect(() => {
    getWinningBid();
  }, [auctionListing, createBidOrOffer]);
              ...
              <div className={styles.descriptionContainer}>
                ...
                <div className={styles.traitsContainer}>
                  ...
                  {winningBid && (
                    <div>
                      <h3>Winning Bid</h3>
                      <p>Auction ID: {winningBid.auctionId}</p>
                      <p>Bidder Address: {winningBid.bidderAddress}</p>
                      <p>Bid Amount: {winningBid.bidAmount}</p>
