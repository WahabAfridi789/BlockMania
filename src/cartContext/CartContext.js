...
export const CartContextProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    if (userId) {
      const storedUserCartItems = localStorage.getItem(`userCartItems_${userId}`);
  ...
  const updateLocalStorage = (items) => {
    if (userId) {
      localStorage.setItem(`userCartItems_${userId}`, JSON.stringify(items));
    }
  };
  ...
  const removeFromCart = (itemId) => {
    console.log(itemId)
    const updatedCartItems = cartItems.filter((item) => item.metadata.id !== itemId);
